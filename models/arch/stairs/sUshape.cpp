...
float stepSize = 0.3;				//	0.001
float stepHeight = 0.03;			//	0.001
...
int generateModel()
{
	float avRise = (max_rise - min_rise) / 2.0;
	stepCount = floorHeight / avRise;
	float errPerStep = (floorHeight - stepCount * avRise) / stepCount;
	stepHeight = avRise + errPerStep;
	ushape(z, -b, stepCount, stairsWidth, midSize, stepsPerSegment);
}
